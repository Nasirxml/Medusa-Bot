...
var Ownerin = `200630907986124@lid`
var ownerNumber = [`200630907986124@lid`, `${nomerOwner}@s.whatsapp.net`,`${conn.user.id}`]
...
const Input = Array.isArray(mentionByTag) && mentionByTag[0] ? mentionByTag[0] : mentionByReply ? mentionByReply : q ? numberQuery : false
...
if (m.isGroup) {
  ...
  let isOwner = m.sender.includes(global.nomerOwner)

  // Cek apakah grup ini tidak terdaftar dan bukan owner
  if ((!chat.registered || chat.expired === 0) && !isOwner) {
    if (global.session === '.session' || global.session === 'sessions') return

    await conn.sendMessage(m.chat, {
      text: `Group ini tidak terdaftar di dalam database order bot
