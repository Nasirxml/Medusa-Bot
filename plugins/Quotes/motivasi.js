...
let handler = async(m, { conn }) => {
    ...
    let ppnyauser = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/89c1638d9620584e6e140.png');
    
    const tweet = await new canvafy.Tweet()
        .setTheme("dim")
        .setUser({ displayName: `${name}`, username: 'motivasi'})
        .setVerified(true)
        .setComment(motivasii)
        .setAvatar(ppnyauser)
        .build();

    conn.sendMessage(m.chat, { image: tweet, caption: `${motivasii}` }, { quoted: m });
};
