...
Contoh : ${usedPrefix + command} https://vt.tiktok.com/ZS6QGKCyC/`;
    ...
    try {
        let { slides, dlink } = await ttsave.video(text);

        for (let slide of slides) {
        await conn.sendFile(m.chat, slide.url, `slide-${slide.number}.jpg`, `${slide.number}`, m);
                await sleep(1500);
            }
...
handler.command = /^(ttslide|tiktokslide|ttdl|tiktokdl)$/i;
handler.limit = true;
