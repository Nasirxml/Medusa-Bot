...
let handler = async (m, { conn, args, usedPrefix, command, text }) => {
    ...
    try {
        let res = await directScrape(text);
        
        if (!res.status) {
            return m.reply(res.message || 'Error fetching data.');
        }
        ...
        if (!isVideo) {
            await m.reply('Terdeteksi url foto instagram\nFoto dikirim ke chat pribadi');
            for (let i of data.url) {
                await sleep(2000);
                conn.sendMessage(m.sender, { image: { url: i } }, { quoted: m });
            }
        } else {
            await m.reply('Terdeteksi url video instagram');
